...
@php
    ...
    if (is_string($aspect) && array_key_exists($aspect, $aspectPresets)) {
        $aspectRatio = $aspectPresets[$aspect];
    } elseif (is_string($aspect) && preg_match('/^\d+\s*\/\s*\d+$/', trim($aspect))) {
        $aspectRatio = preg_replace('/\s+/', ' ', trim($aspect));
    } else {
        $aspectRatio = $aspectPresets['square'];
    }
    ...
    $minWidth = $minWidth !== null ? max(1, (int) $minWidth) : null;
    $minHeight = $minHeight !== null ? max(1, (int) $minHeight) : null;
    $maxWidth = $maxWidth !== null ? max(1, (int) $maxWidth) : null;
    $maxHeight = $maxHeight !== null ? max(1, (int) $maxHeight) : null;
    ...
    $inputId = $id ?? ($name ? 'input-image-'.$name : 'input-image-'.str()->uuid());
    ...
    $dropText = $dropText ?? 'Arraste e solte a imagem aqui';
    ...
    $constraintsHint = collect([
        ...
        $minWidth || $minHeight
            ? 'Mín. '.($minWidth ?? '…').'×'.($minHeight ?? '…').'px'
            : null,
        $maxWidth || $maxHeight
            ? 'Máx. '.($maxWidth ?? '…').'×'.($maxHeight ?? '…').'px'
            : null,
...
<div
    x-data="formInputImage(@js($alpineConfig))"
    x-ref="root"
...
>
    ...
    @elseif ($layout === 'dropzone')
        ...
        @if ($showMeta)
            <ul class="m-0 flex list-none flex-col gap-1 p-0" x-show="hasImages" x-cloak>
                <template x-for="(item, index) in existing" x-bind:key="'meta-existing-' + index">
                    <li class="truncate text-xs text-muted-foreground">
                        <span x-text="item.name"></span>
                        <span x-text="' · ' + existingMeta(item)"></span>
                    </li>
                </template>
                <template x-for="(item, index) in items" x-bind:key="'meta-' + item.id">
                    <li class="truncate text-xs text-muted-foreground">
                        <span x-text="item.name"></span>
                        <span x-text="' · ' + itemMeta(item)"></span>
                    </li>
                </template>
            </ul>
        @endif
