...
@php
	...
	if (isset($errors) && $errors instanceof ViewErrorBag) {
		$errorBag = $errors;
	} elseif (view()->shared('errors') instanceof ViewErrorBag) {
...
>
	...
	<div @class([
		...
		<div @class([
			...
			@if ($hasIconStart)
				<span class="relative z-10 inline-flex shrink-0 items-center justify-center {{ $stateTextClasses }}"
				      aria-hidden="true">
		...
		<div class="flex items-start justify-between gap-3">
			...
			@if ($showCounter)
				<p
					...
					@endif
				>@unless ($needsAlpine)
						{{ strlen((string) ($value ?? '')) }}@if ($maxlength !== null)
							/{{ $maxlength }}
						@endif
					@endunless</p>
			@endif
