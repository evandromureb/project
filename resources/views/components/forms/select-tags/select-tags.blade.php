...
@php
    ...
    $inputId = $id ?? ($name ? 'select-tags-'.$name : 'select-tags-'.str()->uuid());
...
<div
    x-data="formSelectTags(@js($alpineConfig))"
    x-modelable="value"
    x-ref="root"
...
>
    ...
    @if (filled($name))
        <template x-for="(item, index) in value" x-bind:key="'st-hidden-' + index + '-' + item">
    ...
    >
        ...
        <div @class(['relative min-w-0 flex-1', $floating ? 'self-stretch' : null])>
            <div @class([
                ...
                <template x-for="opt in visibleTags" x-bind:key="'st-chip-' + opt.value">
                ...
                <button
                    ...
                    class="inline-flex items-center font-medium {{ $chipToneClasses }} {{ $chipSizeClasses }} {{ $chipRadiusClasses }} cursor-pointer opacity-90 transition-opacity hover:opacity-100"
                    x-bind:aria-label="'Mostrar ' + hiddenTagCount + ' {{ $moreText }}'"
                >
                ...
                <button
                    type="button"
                    x-show="expanded && maxVisible !== null && count > maxVisible"
                    x-cloak
                    @click.stop="toggleExpanded()"
                    class="inline-flex items-center font-medium text-muted-foreground {{ $chipSizeClasses }} {{ $chipRadiusClasses }} cursor-pointer hover:text-foreground"
                >
                    Menos
                </button>
                ...
                <input
                    ...
                    x-bind:placeholder="hasValue ? '' : @js($floating ? ' ' : $placeholder)"
        ...
        @else
            ...
            @if ($clearable)
                <button
                    type="button"
                    x-show="hasValue && canEdit && ! loading"
                    x-cloak
                    @click.stop="clear()"
    ...
    <template x-teleport="body">
        ...
        >
            <div
                x-show="selectAll && (! isRemote || filteredOptions.length > 0)"
                x-cloak
            ...
            <div class="max-h-60 overflow-y-auto py-1">
                ...
                <template x-for="(item, index) in visibleItems" x-bind:key="'st-item-' + index + '-' + (item.type === 'group' ? item.label : item.option.value)">
                    <div>
                        ...
                        <template x-if="item.type === 'option'">
                            ...
                            >
                                <span
                                    class="mt-0.5 flex size-4 shrink-0 items-center justify-center rounded border text-[10px]"
                                    x-bind:class="isSelected(item.option.value)
    ...
    @if ($hasError || $hasHint || $showCounter)
        <div class="flex items-start justify-between gap-3">
            ...
            @if ($showCounter)
                ...
                >{{ count($initialValue) }}{{ $maxSelected !== null ? '/'.(int) $maxSelected : '' }}</p>
            @endif
        </div>
    @endif
</div>
