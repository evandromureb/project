...
class MenuTree
{
    ...
    public function flatten(array $tree, ?string $parentKey = null): array
    {
        ...
        foreach (array_values($tree) as $sort => $node) {
            $key = $node['name'] ?? null;

            if (! is_string($key) || $key === '') {
                throw new InvalidArgumentException('Cada nó da árvore precisa de um name (key) válido.');
            }
     ...
     */
    public function resolveKey(string $group, array $item, ?Menu $parent = null): string
    {
